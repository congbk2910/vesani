...
class Data extends AbstractHelper
{
    ...
    public function getObject($entity)
    {
        ...
        if ($entity === EntityType::POSTS_BY_MAGEPLAZA_BLOG) {
            return ObjectManager::getInstance()->create(\Mageplaza\Blog\Model\Post::class);
        }
        ...
        foreach ($match[1] as $key => $attribute) {
            if ($type === Type::PAGES) {
                ...
                if ($attribute === 'category_name' && is_array($product->getCategoryIds())
                    && count($product->getCategoryIds())) {
                    $randomCategory               = array_rand($product->getCategoryIds());
                    $productData[$match[0][$key]] = $this->categoryFactory->create()
                        ->load($product->getCategoryIds()[$randomCategory])->getName();
                } elseif ($product->getResource()->getAttribute($attribute)
                    && $product->getResource()->getAttribute($attribute)->usesSource()) {
                ...
                } elseif (in_array($attribute, ['price', 'special_price'], true)) {
    ...
    {
        $blog = ObjectManager::getInstance()->create(\Mageplaza\Blog\Model\Post::class)
            ->getCollection()->setPageSize($limit);
        $blog->getSelect()->orderRand();
     ...
     * @param array $dataAttributeFilter
     * @param bool $isKeyWord
     *
     * @return string
     */
