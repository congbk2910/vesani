...
use Mageplaza\SeoRule\Model\ResourceModel\Meta\CollectionFactory;
...
class Save extends Rule
{
     ...
     * @param CollectionFactory $metaCollectionFactory
    ...
    public function __construct(
        ...
        CollectionFactory $metaCollectionFactory,
        Js $backendDecode
    ) {
    ...
    public function execute()
    {
        ...
        if (!empty($data)) {
            try {
                ...
                if (isset($data['page'])) {
                    $data['rule']['pages'] =
                        $this->helperData->serialize($this->backendDecode->decodeGridSerializedInput($data['page']));
                }
                ...
                if (!empty($isApply) && $isApply == 'saveAndApply' && $model->getStatus()) {
                    $metaCollection = $this->metaCollectionFactory->create();
                    $metaTable      = $metaCollection->getMainTable();
                    $metaModel      = $this->metaFactory->create();
                    $metaCollection->getConnection()->delete($metaTable, ['rule_id = ?' => $model->getId()]);
                    $this->helperData->applyRuleId($model, $metaModel);
