...
class CategoriesTree extends Template implements IdentityInterface
{
     ...
     * @return string
     */
    public function getHtml($outermostClass = '', $childrenWrapClass = '', $limit = 15, $displayType = 0)
    {
        ...
        $colStops = [];
        if ($limit) {
            $colStops = $this->_columnBrake($this->_menu->getChildren(), $limit);
        }
        $count = $this->_menu->getChildren()->count();
        $class = "";
        if($count >= $limit){
            $class = "groupmenu-width-24";
        }
        if($displayType == 0){
            $html = '<ul class="cat-tree groupmenu-drop slidedown '.$class.'">';
        }elseif($displayType == 1){
            $html = '<ul class="cat-tree groupmenu-nondrop slidedown '.$class.'">';
        }
        
        $html .= $this->_getHtml($this->_menu, $childrenWrapClass, $limit, $colStops);
        $html .= '</ul>';
    ...
    protected function _columnBrake($items, $limit)
    {
        ...
        foreach ($items as $item) {
            $place = 1;//$this->_countItems($item->getChildren()) + 1;
            $count += $place;
